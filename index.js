...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511989677033:+55 (62) 9663-8900\n' // ID do WhatsApp + nÃºmero de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
	    ...
	    case 'kapankah':
			...
			case 'closegc':
				...
				case 'toimg':
                	...
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('âŒ responder vÃ­deo hum âŒ')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('âŒ Falha ao converter vÃ­deo para mp3 âŒ')
						buffer = fs.readFileSync(ran)
               ...
               if (anu.error) return reply(anu.error)
				...
				if (isUser) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('âŒ *NSFW NAUM ATIVADO* âŒ')
                                                if (!isUser) return reply(mess.only.daftarB)
